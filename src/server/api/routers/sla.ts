...
export const slaRouter = createTRPCRouter({
  ...
  getPagination: protectedProcedure.query(async ({ ctx }) => {
    ...
    const requestTypes = ['RFC_Change_App', 'RFC_Change_API', 'Permintaan_New_App_or_Major', 'Permintaan_New_API_or_Major'];

    const data = requestTypes.map((type, index) => {
      const filtered = requests.filter((r) => r.serviceType === type);
      const projectDone = filtered.filter((r) => r.project?.status === 'DONE');
      ...
      const percentage = projectDone.length === 0 ? 100 : Math.round((achieved / projectDone.length) * 100);
      ...
      return {
        ...
        status: percentage >= 97 ? 'ACHIEVED' : 'NOT ACHIEVED'
      };
    });
  ...
  getSlaByType: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const percentage = data.length === 0 ? 100 : Math.round((achieved / data.length) * 100);

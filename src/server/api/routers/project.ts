...
export const projectRouter = createTRPCRouter({
  ...
  getPagination: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      statusCounts.forEach((item) => {
        formattedStatusCounts[item.status as ProjectStatus] = item._count.status;
      });
  ...
  startProject: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const dueDate = project.request?.slaDays ? new Date(startedAt.getTime() + project.request?.slaDays * 24 * 60 * 60 * 1000) : null;

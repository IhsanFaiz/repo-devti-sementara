...
export default function AlertItemDelete({ item, open, handleClose }: Props) {
  ...
  const deleteRequest = api.request.delete.useMutation({
    ...
    onError: (ctx) => {
      openSnackbar({
        open: true,
        message: ctx.message || 'Error deleting request.',
        variant: 'alert',
        alert: {
          color: 'error'
        }
      } as SnackbarProps);
      console.error('Error deleting request:', ctx.message);
    }
  });

  const deletehandler = async () => {
    try {
      await deleteRequest.mutateAsync({ id: item?.id! });
    } catch (error) {
        ...
        <Stack alignItems="center" spacing={3.5}>
          ...
          <Stack spacing={2}>
            ...
            <Typography align="center">
              ...
              request, all data associated with that request will also be deleted.
            </Typography>
          </Stack>
          ...
          <Stack direction="row" spacing={2} sx={{ width: 1 }}>
            ...
            <Button fullWidth color="error" variant="contained" onClick={deletehandler} disabled={deleteRequest.isPending} autoFocus>

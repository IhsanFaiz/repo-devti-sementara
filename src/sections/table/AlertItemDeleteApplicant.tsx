// File: src/sections/table/AlertItemDeleteApplicant.tsx
...
export default function AlertItemDelete({ item, open, handleClose }: Props) {
  ...
  const deleteApplicant = api.applicant.delete.useMutation({
    onSuccess: () => {
      utils.applicant.getAll.invalidate();
    ...
    onError: (ctx) => {
      openSnackbar({
        open: true,
        message: ctx.message || 'Error deleting applicant.',
        variant: 'alert',
        alert: {
          color: 'error'
        }
      } as SnackbarProps);
      console.error('Error deleting applicant:', ctx.message);
    }
  });

  const deletehandler = async () => {
    try {
      await deleteApplicant.mutateAsync({ id: item?.id! });
    } catch (error) {
        ...
        <Stack alignItems="center" spacing={3.5}>
          ...
          <Stack spacing={2}>
            ...
            <Typography align="center">
              By deleting applicant
              <Typography variant="subtitle1" component="span">
                {' '}
                {item?.firstName} {item?.lastName}{' '}
              </Typography>
              , all data associated with this applicant will also be deleted.
            </Typography>
          </Stack>
          ...
          <Stack direction="row" spacing={2} sx={{ width: 1 }}>
            ...
            <Button fullWidth color="error" variant="contained" onClick={deletehandler} disabled={deleteApplicant.isPending} autoFocus>

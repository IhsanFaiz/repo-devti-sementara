    ...
    <>
      <div>
        <Link href="/request" className="flex items-center gap-2">
          <ArrowLeft />
          <Typography variant="h5">Back to App Requests</Typography>
        </Link>
      </div>
          ...
          {isLoading ? (
            ...
            <>
              <MainCard>
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <Typography variant="h4">{request?.references}</Typography>
                    <Chip
                      label={request?.status}
                      color={request?.status === 'APPROVED' ? 'success' : request?.status === 'PENDING' ? 'default' : 'error'}
                    ></Chip>
                  </div>
                  <div className="flex gap-3">
                    <Chip
                      label={request?.priority}
                      color={request?.priority === 'MEDIUM' ? 'warning' : request?.priority === 'LOW' ? 'default' : 'error'}
                    ></Chip>
